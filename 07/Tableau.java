...
public class Tableau {
    ...
    public void addValue(Integer value) {
        if (smallest == null) { // if tableau empty, create first cell
            smallest = new Cell(value);
            return;
        }
        // create new cell at end of row, return a bumped value
        Integer next = addToRow(smallest, value);
        Cell row = smallest;
        // keep bumping down until at bottom
        while(next != null){
            // if row below has cells
            if (row.below != null){
                row = row.below;
                next = addToRow(row, next);
            } else{    // if row below is empty
                Cell newCell = new Cell(next);
                newCell.above = row;
                row.below = newCell;
                next = null;
            }
    ...
    protected Integer addToRow(Cell curr, int value) {        
        // follow right-pointing links until null or greater value
        while (curr.right != null && curr.value < value){
            curr = curr.right;
        }
        // if need to bump a cell
        if (curr.value > value){
            Integer bumpVal = curr.value;
            curr.value = value;
            return bumpVal;
        }
